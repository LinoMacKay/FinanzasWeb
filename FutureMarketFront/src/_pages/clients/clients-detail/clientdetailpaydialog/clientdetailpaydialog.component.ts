...
export class ClientdetailpaydialogComponent implements OnInit {
  ...
  displayedQuoteColumns: string[] = ['value','interest','firstPaidDay','lastPaidDay','actions'];
  @ViewChild(MatPaginator, { static: false }) paginator: MatPaginator;
  @ViewChild(MatSort, { static: false }) sort: MatSort;

  constructor(@Inject(MAT_DIALOG_DATA) public data: QuoteDetails,
  private dialogRef: MatDialogRef<ClientdetailpaydialogComponent>,private quoteDetailService:QuoteDetailService,
  private customerService:CustomerService,private loginService:LoginService,private quoteService:QuoteService,private dialog: MatDialog) { }

  ngOnInit(): void {
    this.User = this.loginService.getUser();

    this.quoteService.UpdateInterest(this.data.quoteDetailsId).subscribe(data=>{
  ...
  openDialog(row) {
    this.dialog.open(PaydebtComponent, {
      width: '400px',
      disableClose: false,
      data: row}
