...
import { Quote } from 'src/_model/quote';
...
export class PaydebtComponent implements OnInit {
  amount:number;
  totalDebt:number;
  constructor(@Inject(MAT_DIALOG_DATA) public data: Quote,private matSnackBar: MatSnackBar,private quoteService:QuoteService,
  ...
  Pay(){
    if(this.amount < this.totalDebt && this.data.interest > 0){
      this.matSnackBar.open('Si existe mora, debe hacer un pago total','Aceptar',{
        duration:2000
      });
    }
    else if(this.amount > this.totalDebt){
      this.matSnackBar.open('Por favor pague como maximo: ' + this.totalDebt,'Aceptar',{
        duration:2000
      });
    }
    else{
      this.quoteService.PayQuote(this.data.quoteId,this.amount).subscribe(()=>{
